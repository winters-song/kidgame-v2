...
const autohelperdefpat0 = function(trans, move, color, action)
{
  const b = this.board
  return b.accuratelib(move, color, b.MAX_LIBERTIES, null)>1;
}

const autohelperdefpat1 = function(trans, move, color, action)
{
  const a = AFFINE_TRANSFORM(610, trans, move);
  const b = AFFINE_TRANSFORM(685, trans, move);

  return ATTACK_MACRO.call(this, a) && this.play_attack_defend_n(color, 0, 2, [move, b, a]);
}

const autohelperdefpat10 = function(trans, move, color, action)
{
...
const autohelperdefpat31 = function(trans, move, color, action)
{
  const bd = this.board
  const a = AFFINE_TRANSFORM(610, trans, move);
  ...
  return bd.countlib(a)>1 && bd.countlib(b)>1 && bd.countlib(C)<=2 && bd.accuratelib(move, color, bd.MAX_LIBERTIES, null)>1;
}
...
const defpat = data.map(item => new Pattern(item))

export const defpat_db = new PatternDB([-1, 0, defpat, null])